   ...
   # data_loader = torch.utils.data.DataLoader(dataset, batch_size=5, shuffle=True, num_workers=25)
    ...
    for data in data_loader:
        ...
        for l in range(len(labels)):
            ...
            print("path", args.result_path + SCENE_NAMES[labels[l]])
            for root, dirs, files in os.walk(
                    args.result_path + SCENE_NAMES[labels[l]]+'/'):
                if dirs != None:
                    stored_dirs = dirs
                img_name = paths[l].split('/')[1][:-4]
                print("img name {0}".format(img_name))
                if args.train:
                    img_name = paths[l][6:-4]
                if img_name not in stored_dirs:
                    ...
                    if len(techniques) < 4:
                        ...
                        for t in techniques:
                            grounding = gen_grounding(displ_img, t, SCENE_NAMES[labels[l]], 'bam_scene', save_path=args.result_path,
                                                      save=True, unique_id=img_name)
                    else:
                        grounding = gen_all_groundings(displ_img, SCENE_NAMES[labels[l]], 'bam_scene',
                                                       save_path=args.result_path,
                                                       save=True, unique_id=img_name, show=True)
                    print('')
                    break
                else:
                    print('explanation already generatd: {0}'.format(paths[l]))
                    print('')
                    break
